...
func TestBuildSchemaWithGlobalTemporaryTable(t *testing.T) {
	...
	doChange := func(changes ...func(m *meta.Meta, builder *infoschema.Builder)) infoschema.InfoSchema {
		...
		err := kv.RunInNewTxn(ctx, store, true, func(ctx context.Context, txn kv.Transaction) error {
			...
			for _, change := range changes {
				builder := infoschema.NewBuilder(store, nil, nil).InitWithOldInfoSchema(curIs)
				change(m, builder)
				curIs = builder.Build()
			}
			return nil
	...
	require.True(t, ok)
	builder, err := infoschema.NewBuilder(store, nil, nil).InitWithDBInfos([]*model.DBInfo{newDB}, newIS.AllPlacementPolicies(), newIS.AllResourceGroups(), newIS.SchemaMetaVersion())
	require.NoError(t, err)
...
func TestBuildBundle(t *testing.T) {
	...
	builder, err := infoschema.NewBuilder(store, nil, nil).InitWithDBInfos([]*model.DBInfo{db}, is.AllPlacementPolicies(), is.AllResourceGroups(), is.SchemaMetaVersion())
	require.NoError(t, err)
	is2 := builder.Build()
